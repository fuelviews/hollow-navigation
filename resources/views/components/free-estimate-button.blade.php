<button @click="open = !open" onclick="Livewire.dispatch('openModal')" {{ $attributes->merge(['class' => 'inline-flex items-center justify-center gap-2 rounded-md bg-cta-500 px-4 py-2 text-base font-bold text-white hover:bg-cta-600 transition-colors duration-200 whitespace-nowrap cursor-pointer']) }}>
    <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" class="h-5 w-5 flex-shrink-0" viewBox="0 0 384 512">
        <path
    ...
    <span>{{ config('business-info.cta') }}</span>
</button>