<nav x-data="{
     ...
     " :class="{
         'backdrop-blur-xl bg-white/70 text-nav-type {{ Navigation::isRoundedBottomScrolled() ? 'rounded-b-standard' : '' }}': scrolled || {{ Navigation::getPreScrolledRoute() }},
         '{{ Navigation::isTransparentNavBackground() ? 'bg-transparent text-nav-type-trans' : 'backdrop-blur-xl bg-white/70 text-nav-type' }}': !scrolled && !{{ Navigation::getPreScrolledRoute() }}
